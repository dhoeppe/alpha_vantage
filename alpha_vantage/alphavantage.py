...
class AlphaVantage(object):
    ...
    class output_format(object):
        ...
        def _extract_data(cls, data_key, call_response):
            # Formatting for sector performance
            if 'sector' == cls.formatting:
                if isinstance(data_key, list):
                    return {key: {k: cls._percentage_to_float(v) for k, v in
                                  call_response[key].items()} for key in data_key}
            elif 'default' == cls.formatting:
                if data_key is not None:
                    return call_response[data_key]
                else:
                    return call_response

        def _handle_call_response_json_api(cls, self, data_key, meta_data_key, call_response,
                                           output_format):
            data = cls._extract_data(data_key, call_response)
        ...
        def _handle_call_response_csv_api(cls, self, output_format, call_response):
